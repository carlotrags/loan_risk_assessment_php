        ...
        <div class="user-container">
            ...
            <table>
                ...
                <td>
                    ...
                    <td>
                        <span class="badge p-1 px-2 <?= $row['role'] === 'Manager' ?
                    ...
                    <td class="actions-buttons">
                        <a href="user-edit.php?user_id=<?= $row['user_id'] ?>" title="Edit"><button title="Edit" style="color: green;"><i class="fa-solid fa-pen-to-square"></i></button></a>
                    <form method="POST" action="user-activate-deactivate.php" style="display:inline;"
                        onsubmit="return confirm('Are you sure you want to <?= $row['status'] === 'Deactivated' ? 'activate' : 'deactivate' ?> this user?');">
                        <input type="hidden" name="user_id" value="<?= $row['user_id'] ?>">
                        <input type="hidden" name="current_status" value="<?= $row['status'] ?>">
                        <button type="submit" style="color: <?= $row['status'] === 'Deactivated' ? 'green' : 'red' ?>;"
                                title="<?= $row['status'] === 'Deactivated' ? 'Activate' : 'Deactivate' ?>">
